...
def get_street_link(index,street,street_name, driver,args,log_street_link_file):
    ...
    if review_exists(driver) == True:
        ...
        # sleep(5)
        try:
            local_name = driver.find_element(By.XPATH, '//span[@jstcache="899"]').text
        except:
            local_name = None
...
def scrolling(last_review, driver):
    ...
    sleep(4)
    next_review = driver.find_elements(By.XPATH, '//div[@jstcache="652"]')
    return next_review

def scrapy(url,street,args,driver):
    ...
    try:
        picturesmores = driver.find_elements(By.XPATH, './/div[@jstcache="399"]/button[@data-photo-index="3"]')
    ...
    review_parser(response, street, args)
...
#Parsing the reviews
def review_parser(response,street,args):
    reviews_boxes = response.find_all("div",attrs={"jstcache": "652"})
    for num, box in enumerate(reviews_boxes):
        # print(num)
        # box = reviews_boxes[1]
        data = {}
        data['review_id'] = box['data-review-id']
        data['user'] = box.find("span",attrs = {"jstcache":"607"}).text if box.find("span",attrs = {"jstcache":"607"}) else None
        data['guide'] = box.find("span", attrs={"jstcache": "609"}).text if box.find("span", attrs={"jstcache": "609"}) else None
        data['review_text'] = box.find("span", attrs={"jstcache": "541"}).text if box.find("span", attrs={"jstcache": "541"}) else None
        data['review_date'] = box.find('div',class_='DU9Pgb').find_all('span')[2].text if box.find('div',class_='DU9Pgb').find_all('span')[2] else None
        data['review_rating'] = box.find('div',class_='DU9Pgb').find_all('span')[1]['aria-label'].strip() if box.find('div',class_='DU9Pgb').find_all('span')[1]['aria-label'] else None
        data['street'] = street
        pictures = box.find("div", attrs={"jstcache": "399"}).find_all('button') if box.find("div", attrs={"jstcache": "399"}).find_all('button') else None
        picture_urls = []
        if pictures:
            try:
